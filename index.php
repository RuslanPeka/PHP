<?php
    ...
    class dbWork
    {
        ...
        {
            $query = 'SELECT * FROM `customers` WHERE `name` IN (SELECT `name` FROM `customers` GROUP BY `name` HAVING COUNT(*) > 1) ORDER BY `name`';
            $result = $this->conn->query($query);
            if (!$result) {
                echo 'Ошибка выборки из таблицы ' . $tableName . ': ' . $this->conn->error . '<br>';
            } else {
                $row = $result->fetch_array(MYSQLI_ASSOC);
                echo 'Дублируются следующие контрагенты:<br>';
                foreach($row as $v) {
                    print_r($v);
                }
            }
